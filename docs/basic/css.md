...
CSS 预处理器包括：Sass/Less/Postcss。CSS 预处理器的原理: 是将类 CSS 语言通过 Webpack 编译 转成浏览器可读的真正 CSS。
...
CSS3 规范中的要求 :hover 和 :active 等伪类使用单冒号( : )。使用双冒号( :: )表示伪元素，以此来区分伪类和伪元素，比如 ::before 和 ::after 等伪元素使用双冒号( :: )，除了一些低于 IE8 版本的浏览器外，大部分浏览器都支持伪元素的双冒号( :: )表示方法。
